...
- ~~**Turbopack production build hangs.**~~ **Resolved 2026-07-25.** The cause was cubing.js 0.56's bundling; upgrading to 0.63.3 made `next build` complete in ~9s, and the `--webpack` pin is gone. The suspicion recorded here — cubing.js worker bundling, matching the `import.meta.resolve(…) failed` warnings — turned out to be right. Full write-up in `roadmap.md` → "Resolved: one bundler for dev and production".
